...
#Making a generalized model and adding listener, voice and item pair as random effects. I don't have a good and proper explanation why I chose this, it kinda just felt correct...very precise, I know. Model seemed to converge first time around so no issues there.
...
summary(mdl)
#It seems like most variance is covered by voice, which I think makes sense because the voice will probably have a lot of influence on how the sound is percieved and how distinctive the sounds are .
...
# I made a null model, replacing the predictor with 1 so that I could run an anova.
anova(null_mdl,mdl)

#It seems to me that the p-value is smaller than 0.05 which would mean that the predictor has a statistically significant effect
...
predict <- ggeffects::predict_response(mdl, terms = "correct_voicing")

predict_response(mdl)

plot(predict)

#I am not sure if I'm doing this correctly or if I'm missing something, but I'm tired and so I will leave it here.
```
